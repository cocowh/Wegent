...
async def prepare_knowledge_base_tools(
    ...
    This function encapsulates the logic for creating knowledge base tools and
    enhancing the system prompt with knowledge base instructions.

    NOTE:
    - Knowledge base metadata prompt (kb_meta_prompt) is generated by Backend and transported
      via the execution request (e.g. `ExecutionRequest.kb_meta_prompt`). chat_shell must not
    ...
    Returns:
        ...
        kb_meta_prompt is always an empty string here and should be injected by the caller
        via `dynamic_context` if needed.
    """
    extra_tools: list = []
    ...
    from chat_shell.prompts import (
        KB_PROMPT_NO_RAG,
        KB_PROMPT_RELAXED,
    ...
    # Keep KB prompt template fully static (no kb_meta_list placeholder).
    enhanced_system_prompt = f"{base_system_prompt}{kb_instruction}"

    # kb_meta_prompt is generated by Backend and transported via execution request.
    return KnowledgeBaseToolsResult(
        extra_tools=extra_tools,
        enhanced_system_prompt=enhanced_system_prompt,
        kb_meta_prompt="",
